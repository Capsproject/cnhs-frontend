...
export const PageHeader: React.FC<Props> = (props) => {
  ...
  const userProfile : string = useAuthStore.getState().profile ?? '';
  const isAdmin = () => {
  ...
  const navigateToAccountSetting= () => {
    ...
    if(isAdmin()) {
      window.location.href = "/admin/account-setting/" + userId;
    }
  }
  React.useEffect(() => {
    ...
    <div className="flex bg-white justify-center drop-shadow-md flex-row max-md:flex-col max-md:gap-8 items-center px-5 mb-4 py-2 rounded-md">
      ...
      <div className="flex items-center justify-center">
        <Button
          type="primary"
          shape="circle"
          className="mr-2 bg-orange-500 hover:bg-orange-400 focus:bg-orange-200"
          icon={<LogoutOutlined />}
          onClick={AuthService.logOut}
        />
      </div>
      <div className="flex items-center justify-center">
        <Avatar src={<img src={userProfile}/>}  onClick={navigateToAccountSetting} size={50}/>
      </div>
    </div>
  );
};
